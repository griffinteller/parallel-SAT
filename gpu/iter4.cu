...
constexpr int validBlockSizes[] = {1024, 512, 256};
...
__global__ void propagateAndCheck() {
    ...
    for (int i = tid; i < totalLiterals; i += numThreads) {
        clauseLiterals[i] = devClauseLiterals[i];
    }

    for (int i = tid; i < numLiterals; i += numThreads) {
    ...
    while (true) {
        for (int clauseIdx = tid; clauseIdx < numClauses; clauseIdx += numThreads) {
            ...
            for (int i = 0; i < clauseCount; i++) {
                ...
                if (value == TRUE) {
                    numUnassigned = 0; // prevent propagation
                    break;
                }
                if (value == UNASSIGNED) {
                    numUnassigned++;
        ...
        if (tid == 0) anyPropagated = 0;

        __syncthreads();
    }

    // get clause result
    int myClausesSatisfied = 1;
    int myClausesFalse = 0;
    ...
    for (int clauseIdx = tid; clauseIdx < numClauses; clauseIdx += numThreads) {
        ...
        int thisClauseSatisfied = 0;
        int thisClauseFalse = 1;
        ...
        if (!thisClauseSatisfied) myClausesSatisfied = 0;
        if (thisClauseFalse) myClausesFalse = 1;
    }
    ...
    if (tid == 0) {
        ...
        result.allClausesSatisfied = myClausesSatisfied;
        result.anyClausesFalse = myClausesFalse;
    ...
    if (tid > 0) {
        ...
        if (myClausesFalse) result.anyClausesFalse = 1;
        if (!myClausesSatisfied) result.allClausesSatisfied = 0;
...
bool dpllMain() {
    ...
    while (!assignmentStack.empty()) {
        ...
        int numThreads = 128;
        for (int n : validBlockSizes) {
            if (n <= hostFormula.numClauses) {
                numThreads = n;
                break;
            } 
        }
        ...
        // std::cout << "Block size: " << numThreads << std::endl;
        ...
        propagateAndCheck<<<activeInstances, numThreads, sharedMemSize>>>();
